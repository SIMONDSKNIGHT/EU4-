...
def process_image(input_file, output_file):
    ...
    skip=970
    global arrayone 
    arrayone = [0]*len(unique_colors)
    
    for color in unique_colors:
        if counter<skip :
            largest_section = find_largest_contiguous_section(image, color)
            #ultimately this is incredibly inefficient as we could half the search time if I just made it so that all
            if arrayone[counter]!=1:
...
if __name__ == "__main__":
    input_file = "./970.bmp"  # Replace with your BMP file
    output_file = "./output.bmp"  # Replace with the desired output file
    process_image(input_file, output_file)

#if there are still gaps run until 970